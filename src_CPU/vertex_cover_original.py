...
with netns.NetNS(nspid=host_mnet_pid):
    ...
    def initializer(log_file, host_name, neighbor, root_name, node_cnt, **kwargs):
        ...
        cb_params["alpha"] = kwargs["alpha"]
        cb_params["beta"] = kwargs["beta"]
        ...
        while True:
            try:
                ...
                if name1 == host_name:
                    cb_params["neighbor"][ip2] = cb_params["beta"] * float(others_delta)
        ...
        for neighbor in cb_params["neighbor"].keys():
            cb_params["deal"][neighbor] = min(cb_params["neighbor"][neighbor], cb_params["beta"] * cb_params["self_weight"] / len(cb_params["neighbor"]))
    ...
    def stage_initializer(stage, cb_params):  # there is no need for a stage initializer for this algorithm
        if stage % 2 == 0:
            ...
            if cb_params["sum_deal"] * cb_params["alpha"] <= cb_params["beta"] * cb_params["self_weight"]:
    ...
    def callback(src_ip, stage, buf, cb_params, start_flag):
        ...
        if start_flag == True:
            ...
            if stage % 2 == 1:
                if cb_params["sum_delta"] + cb_params["basis_delta_sum"] >= (1 - cb_params["beta"]) * cb_params["self_weight"]:
                    cb_params["join"] = True
                    for neighbor in cb_params["neighbor"].keys():
            ...
            if buf.req_type == NOTIFY_COVER:
                ...
                if buf.flag == FLAG_RAISE and cb_params["self_stuck_state"] == FLAG_RAISE:
                    ...
                    cb_params["deal"][src_ip] = cb_params["deal"][src_ip] * cb_params["alpha"]
                cb_params["sum_deal"] += cb_params["deal"][src_ip]
