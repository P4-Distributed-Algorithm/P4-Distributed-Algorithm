...
class RoutingController(object):
    ...
    def route(self):
        ...
        for sw_name, controller in self.controllers.items():
            ...
            son_port_node_rid1 = self.controllers[sw_name].mc_node_create(1, son_port_list)
            all_port_node_rid2 = self.controllers[sw_name].mc_node_create(2, all_port_list)
            son_port_node_rid3 = self.controllers[sw_name].mc_node_create(3, son_port_list)
            all_port_node_rid4 = self.controllers[sw_name].mc_node_create(2, all_port_list)

            self.controllers[sw_name].mc_node_associate(1, son_port_node_rid1)
            self.controllers[sw_name].mc_node_associate(2, all_port_node_rid2)
            self.controllers[sw_name].mc_node_associate(2, son_port_node_rid3)
            self.controllers[sw_name].mc_node_associate(3, all_port_node_rid4)
            ...
            for neighbor_name in neighbor_list:
                ...
                son_port_node_rid1 = self.controllers[sw_name].mc_node_create(1, son_port_list)
                all_port_node_rid2 = self.controllers[sw_name].mc_node_create(2, all_port_list)
                son_port_node_rid3 = self.controllers[sw_name].mc_node_create(2, son_port_list)
                
                self.controllers[sw_name].mc_mgrp_create((egress_port + 1) * 256 + 1)
                self.controllers[sw_name].mc_mgrp_create((egress_port + 1) * 256 + 2)
                
                self.controllers[sw_name].mc_node_associate((egress_port + 1) * 256 + 1, son_port_node_rid1)
                self.controllers[sw_name].mc_node_associate((egress_port + 1) * 256 + 1, node)
                self.controllers[sw_name].mc_node_associate((egress_port + 1) * 256 + 2, all_port_node_rid2)
                self.controllers[sw_name].mc_node_associate((egress_port + 1) * 256 + 2, son_port_node_rid3)
                node = self.controllers[sw_name].mc_node_create(4, [egress_port])
        ...
        for sw_name, controller in self.controllers.items():
            ...
            print(sw_name, rand_num)
            self.controllers[sw_name].register_write("max_value", 0, rand_num)
